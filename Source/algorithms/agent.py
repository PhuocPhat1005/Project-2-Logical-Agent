...
class Agent:
    ...
    def dfs(self, program):
        ...
        while frontier:  # While there are cells to explore       
            ...
            if self.check_have_poison(current_y, current_x, program.cells[self.y][self.x]):
            ...
            self.y, self.x = current_y, current_x  # Update agent's current position, agent move to new position
                ...
                for dy, dx in directions:
                    ...
                    if 0 <= ny <= program.map_size - 1 and 0 <= nx <= program.map_size - 1:
                        ...
                        for dyy, dxx in directions:
                            ...
                            if 0 <= nyy <= program.map_size - 1 and 0 <= nxx <= program.map_size - 1:
                ...
                for dy, dx in direction:
                    ...
                    if 0 <= ny <= self.map_size - 1 and 0 <= nx <= self.map_size - 1:
                        for dyy, dxx in direction:
                            nyy, nxx = ny + dyy, nx + dxx
                            if 0 <= nyy <= self.map_size - 1 and 0 <= nxx <= self.map_size - 1:
